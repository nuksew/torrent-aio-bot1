  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const reply = async torr => {
      ...
      if (!torr.done) {
        ...
        let sam = "https://tdb.dailysarkarnaukri.workers.dev/${torr.name}";
        mess1 += `Link: ${torr.downloadLink}\n\nencodeURI(sam)`;
        mess1+= encodeURI(sam)    
        clearInterval(torrInterv);
        torrInterv=null;  
      }
      mess1 += `\n\n Magnet URI: ${torr.magnetURI}`;
      ...
      try {
        ...
        torrInterv = setInterval(async () => {
          ...
          if (!torre.done) {
            ...
            var sam = "https://tdb.dailysarkarnaukri.workers.dev/${torre.name}"; 
            mess1 += `Link: ${torre.downloadLink}\n\n encodeURI(sam)`;
            mess1+= encodeURI(sam)  
            clearInterval(torrInterv);
          }
          mess1 += `\n\n Magnet URI: ${torre.magnetURI}`;
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      if (!torr.done) {
        ...
        var sam = "https://tdb.dailysarkarnaukri.workers.dev/${torr.name}";
        mess1 += `Link: ${torr.downloadLink}\n\nencodeURI(sam)`;
        mess1 += encodeURI(sam)    
      }
      mess1 += `\n\n Magnet URI: ${torr.magnetURI}`;
